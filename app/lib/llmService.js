...
export const callLLM = async (messages, options = {}) => {
  // 从环境变量读取配置
  const apiKey = process.env.NEXT_PUBLIC_LLM_API_KEY || localStorage.getItem('llm_api_key');
  const baseUrl = process.env.NEXT_PUBLIC_LLM_BASE_URL || localStorage.getItem('llm_base_url') || 'https://api.openai.com/v1';
  const defaultModel = process.env.NEXT_PUBLIC_LLM_MODEL || localStorage.getItem('llm_model') || 'gpt-3.5-turbo';
...
请用中文回答，结构清晰，分点列出，专业但通俗易懂，不要使用Markdown格式。
,
...
请用中文回答，结构清晰，分点列出，专业但通俗易懂，重点突出风险点，不要使用Markdown格式。
,
...
请用中文回答，结构清晰，分点列出，专业但通俗易懂，建议要具体可执行，不要使用Markdown格式。
