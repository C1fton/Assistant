...
const analysisFrame = [
  ...
  '所有买入/加仓金额必须受可用现金约束；如果可用现金为 0 或缺失，只能建议观察、减仓、换仓或等待，不得输出新增买入金额。'
];
...
export const PROMPT_TEMPLATES = {
  ...
  rebalanceAdvice: joinPrompt([
    ...
    '建议金额必须基于输入里的 totalAmount、holdingAmount、ratio 和可用资金快照推导。加仓/新建仓合计不得超过 availableCash；若 availableCash 为 0，只能给持有、减仓、换仓或观察建议。',
